...
function TableController({ isOpen, setControllerOpen, items, Item, setItems, count, Edit, SetEdit, SurveyType }) {
    const [TableData, setTableData] = useState({});

    useEffect(() => {
        if (!Item.id) return;
        if (!TableData?.title || (!TableData.MemberSatificaitonMatris && (!TableData?.data?.rows?.length > 0 || !TableData?.data?.columns?.length > 0))) {
