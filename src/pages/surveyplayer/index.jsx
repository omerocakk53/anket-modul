...
export default function SurveyPlayer({
  ...
  const { currentIndex, submitted, goNext, goBack, setSubmitted } =
    useSurveyNavigation({
      data,
      variables,
      answers,
    ...
    <div className="w-screen h-screen overflow-hidden flex flex-col items-center justify-center bg-neutral-dark">
      {tester && (
        <div className="sticky top-0 left-0 w-full mb-2 bg-danger text-white text-center py-2 px-4 text-sm font-medium shadow-md z-50 select-none">
          Siz anketinizin <strong>ön izlemesini</strong> görmektesiniz. Bu
          durumda hiç bir yanıt kaydolmayacaktır. Ön izleme durumundan çıkmak ve
          yanıt kaydı imkanı için{" "}
          <a
            href={`/anket/${slug}`}
      ...
      <div
        className={`w-full h-full shadow-lg relative flex flex-col flex-grow`}
      >
        {!submitted && (
          <>
