...
import DateSelector from "./AnswerCompareComponent/DateSelector.jsx";
import PeriotSelector from "./AnswerCompareComponent/PeriotSelector.jsx";
import Modal from "./AnswerCompareComponent/popUp/Modal.jsx";
...
export default function AnswerComparison({ survey, answers }) {
  ...
  const isGraphSupported = ["rating", "scale"].includes(
    selectedItemId?.split("-")[0],
  );

  const chartRef = useRef(null);
  const tableRef = useRef(null);

  const [colors, setColors] = useState(["#34d399", "#60a5fa"]);
  ...
  const handleDownload = () => {
    if (ViewType === "table") {
      ...
      // Grafik indir
      domtoimage
        .toPng(chartRef.current, { cacheBust: true, bgcolor: "#fff" })
        .then((dataUrl) => {
          ...
          return domtoimage.toPng(tableRef.current, {
            cacheBust: true,
            bgcolor: "#fff",
          });
        })
        ...
        <div className="flex flex-wrap items-center gap-3 justify-start mb-6">
          <div className="flex gap-3 items-center flex-wrap">
            {[
              ...
              {
                key: "horizantalbar",
                icon: <PiChartBarHorizontalDuotone size={20} />,
                label: "Yatay Bar",
              },
              {
                key: "line",
