...
export default function useGuidance(
  items,
  finishWelcomeItems,
  variables,
  setVariables,
  ...
  function handleQuestionGuidance(logics, isOpen) {
    setquestionsGuidanceMenuData({
      items,
      item: null,
      finishWelcomeitems: null,
      isOpen,
    });

    if (Array.isArray(logics) && logics.length) {
      const invalidLogic = logics.find(
        (l) => !items.some((item) => item.id === l.itemId),
      );
      if (invalidLogic) {
      ...
      logics.forEach((logic) => {
        ...
        if (index !== -1) {
          ...
          if (!isSame) {
            ...
            toast.success(
              `Mantık güncellendi: ${logic.expectedValue || logic.conditionType}`,
            );
          }
        } else {
          // Yeni ekle
          ...
          toast.success(
            `Mantık eklendi: ${logic.expectedValue || logic.conditionType}`,
          );
        }
      });
