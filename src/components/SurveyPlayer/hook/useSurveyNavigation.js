...
export default function useSurveyNavigation({ data, variables, answers, submitAnswersFn, surveyId, user, startDate, answersave }) {
    ...
    const checkTableMatch = (answerValue, expectedValue) => {
        if (!answerValue) return false;
        const [expectedRow, expectedCol] = expectedValue.split("|").map(s => s.trim());
        if (typeof answerValue === 'object' && !Array.isArray(answerValue)) {
        ...
        if (Array.isArray(answerValue)) {
            const cell = answerValue.find(cell => cell.rowId === expectedRow && cell.colId === expectedCol);
            return cell !== undefined && cell.value !== undefined && cell.value !== null && cell.value !== "";
        }
        return false;
    };
    ...
    const canDisplayQuestion = (questionId) => {
        ...
        if (question.id.includes('welcome') || question.id.includes('finish')) return true;
        const relatedRules = variables.filter(
            v => v.trueDestinationItemId === questionId || v.falseDestinationItemId === questionId
        );
            ...
            switch (rule.conditionType) {
                ...
                case 'match':
                    if (!answer) matched = false;
                        ...
                        matched = answer.value.some(item =>
                            item.title === rule.expectedValue || item.url === rule.expectedValue
                        );
                    } else {
    ...
    const goNext = async () => {
        ...
        let nextIndex = null;
        const relatedVariables = variables.filter(v => v.itemId === currentItem?.id);
        for (const variable of relatedVariables) {
            let goToItemId = null;
            let conditionMatched = false;
            if (variable.conditionType === "match") {
                if (currentItem.type === "Matris" && variable.conditionType === "match") {
                    conditionMatched = checkMatrixMatch(currentValue, variable.expectedValue);
                } else if (currentItem.type === "Table" && variable.conditionType === "match") {
                    conditionMatched = checkTableMatch(currentValue, variable.expectedValue);
                } else {
            ...
            else if (variable.falseDestinationItemId) goToItemId = variable.falseDestinationItemId;
            if (goToItemId) {
        ...
        if (nextIndex !== null) {
            ...
            if (nextIdx < data.length) {
                ...
                await submitAnswersFn(answers, data, surveyId, user, startDate, answersave);
                setSubmitted(true);
            }
        } else {
            await submitAnswersFn(answers, data, surveyId, user, startDate, answersave);
