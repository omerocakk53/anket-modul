...
const questionLabels = [
    'Kısa Metin', 'Çoktan Seçmeli', 'Uzun Metin', 'Resimli Çoktan Seçmeli',
    'Soru Grubu', 'Açılan Liste', 'Sayısal Cevap', 'Değerlendirme Ölçeği',
    'E-posta', 'Derecelendirme', 'Bağlantı/Site Adresi', 'Sıralama',
    'Dosya Yükleme', 'Matris', 'Tablo',
];

export default function QuestionRenderer({ item, answers, onChange }) {
    if (!item) return null;

    const questionItems = Object.values(answers).filter(i => questionLabels.includes(i.label));
    ...
    switch (item?.label) {
        ...
        case 'Tablo': return <Table {...commonProps} />;
        default: return null;
    }
}
