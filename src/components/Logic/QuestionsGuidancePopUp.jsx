...
const QuestionsGuidancePopUp = ({
  ...
  useEffect(() => {
    if (guidance.item) {
      ...
      const existing = variables
        .filter((v) => v.itemId === guidance.item.id)
        .map((v) => {
          if (
            guidance.item.type === "Matris" ||
            guidance.item.type === "Table"
          ) {
      ...
      if (existing.length) {
        ...
        if (guidance.item.type === "Matris" || guidance.item.type === "Table") {
          setLogics([
            { ...emptyLogic, itemId: guidance.item.id, Row: "", Col: "" },
          ]);
        } else {
  ...
  const onAdd = () => {
    if (item.type === "Matris" || item.type === "Table") {
      setLogics((prev) => [
        ...prev,
        { ...emptyLogic, itemId: item.id, Row: "", Col: "" },
      ]);
    } else {
    ...
    <div className="fixed inset-0 backdrop-blur-sm flex justify-center items-center z-50">
      <div className="bg-white rounded-md shadow-lg w-[80%] p-5 max-h-[80vh] overflow-y-auto">
        ...
        <div className="bg-gray-50 p-3 rounded mb-4 text-sm text-gray-600 flex flex-col gap-1">
          <div>
            <span className="font-medium">Komponent:</span> {item.label}
          </div>
          <div>
            <span className="font-medium">Soru:</span> {item.title}
          </div>
        </div>
          ...
          <div key={idx} className="flex items-center gap-2 mb-4 flex-wrap">
            {isOpenEnded ? (
              <select
                ...
                onChange={(e) =>
                  onChangeLogic(idx, "conditionType", e.target.value)
                }
                ...
                {item.type === "Matris" || item.type === "Table" ? (
                  <>
                    <select
                      value={logic.Row || ""}
                      onChange={(e) =>
                        onChangeLogic(idx, "Row", e.target.value)
                      }
                      className="border border-gray-300 rounded px-2 py-1 text-sm"
                    >
                    ...
                    <select
                      value={logic.Col || ""}
                      onChange={(e) =>
                        onChangeLogic(idx, "Col", e.target.value)
                      }
                      className="border border-gray-300 rounded px-2 py-1 text-sm"
                    >
            ...
            <select
              ...
              onChange={(e) =>
                onChangeLogic(idx, "trueDestinationItemId", e.target.value)
              }
              className="border border-gray-300 rounded px-2 py-1 text-sm w-[160px]"
            >
              <option value="">Seçiniz</option>
              {[
                ...items.filter((itm) => itm.id !== item.id),
                ...finishWelcomeitems,
              ].map((itm, index) => (
            ...
            <select
              ...
              onChange={(e) =>
                onChangeLogic(idx, "falseDestinationItemId", e.target.value)
              }
              className="border border-gray-300 rounded px-2 py-1 text-sm w-[160px]"
            >
              <option value="">Seçiniz</option>
