...
export default function CreateNewGroupModal({ isOpen, onClose, onCreate }) {
  ...
  const handleNextStep2 = (e) => {
    ...
    // description isteğe bağlı olduğu için kontrol etmiyoruz ama istersen ekleyebilirsin
    setCurrentStep(3);
  };

  const handleCreate = () => {
    onCreate({ newGroupName: newGroupName.trim(), title: title.trim(), description: description.trim() });
    // opsiyonel: modalı kapat veya state temizle
    setCurrentStep(1);
    setNewGroupName('');
    setTitle('');
    setDescription('');
        ...
        {currentStep === 3 && (
          <div className="space-y-6">
            ...
            <div className="bg-neutral-100 p-4 rounded-xl space-y-2 border border-neutral-300">
              <p>
                <strong>Klasör Adı:</strong> <span className="text-primary">{newGroupName}</span>
              </p>
              <p>
