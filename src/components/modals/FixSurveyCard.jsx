...
export default function FixSurveyCard({
  AddAnimateRef,
  edit,
  survey,
  onSelect,
  ...
  return (
    <div
      ref={AddAnimateRef}
      className="bg-white rounded-lg shadow-md border p-5 flex flex-col justify-between hover:shadow-lg transition-shadow"
    >
      <div>
        <h3 className="text-lg font-bold text-neutral-darkest truncate">
          {survey.title}
        </h3>
          ...
          <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
            <div className="bg-white w-full max-w-5xl rounded-2xl shadow-2xl p-8 relative animate-fade-in">
              ...
              <h3 className="text-2xl font-semibold text-center text-neutral-800 mb-6">
                <span className="truncate">"{survey.title}"</span> Anket Ön
                İzleme
              </h3>
              <hr className="mb-4" />
              <div className="flex flex-col gap-4 h-[300px] mb-5 overflow-auto">
                {survey.items.map((item, index) => (
                  <div
                    key={index}
                    className=" w-full max-w-5xl p-5 relative animate-fade-in"
                  >
                    <div>
                      <span>{index + 1}. Soru</span>
                      <h3 className="text-lg font-semibold text-neutral-800 mb-4 truncate">
                        {item.title}
                      </h3>{" "}
                      - Soru Tipi ({item.label})
                    </div>
                    <hr className="mt-1" />
                  </div>
                ))}
