...
function AnswerChartSwitcher(props) {
  const {
    view, setView, labels, data,
    questionType, rawCounts,
    questionMemberSatificaitonMatris, tableData
  } = props;
  ...
  const { handleDownload, chartRef, tableRef } = useDownload(view, questionType);
    ...
    <div className="p-6 max-w-screen-xl mx-auto bg-white rounded-lg shadow-lg">
      ...
      <div className="flex flex-wrap items-center gap-3 justify-start mb-6">
        {[
          ...
          { key: 'horizantalbar', icon: <PiChartBarHorizontalDuotone />, label: 'Yatay Bar' },
