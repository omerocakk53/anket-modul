                ...
                <div className="w-full border-secondary-light border p-4 rounded-lg text-primary-darktext bg-neutral/50 space-y-1 shadow-sm">
                    {surveyType === false ? (
                        ...
                        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                            <SortableContext
                                items={items.filter(item => !item.id.includes("welcome") && !item.id.includes("finish")).map(i => i.id)}
                                        ...
                                        .map((item, index) => (
                                            <div key={item.id} className="w-full">
                                                <ComponentViewItems
                                                    ...
                                                    count={`${index + 1} - ${item.label}`}
                                                    onClick={() => handleEdit(item)}
                                                    questionguidance={() => setquestionsGuidanceMenuData({ items, item, finishWelcomeitems: finishWelcomeItems || [], isOpen: true })}
                                                />
                                            </div>
                                        ))}
                                </div>
